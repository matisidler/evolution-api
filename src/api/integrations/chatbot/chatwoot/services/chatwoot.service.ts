    ...
    while (attempts < maxAttempts) {
      try {
        ...
        if (attempts === maxAttempts) {
          this.logger.error(error);
          throw new Error(`Failed to send attachment after ${maxAttempts} attempts: ${error?.toString() || error}`);
        }
        this.logger.warn(`Send attachment attempt ${attempts} failed, retrying in 0.5s...`);
